...
%% This section creates a frame around the blood vessel.
...
%% Creating empty vectors for plotting the sum of agents through iterations 
V1=[];
V2=[];
V3=[];
V4=[];
%% 1. Interaction rules between agents 
...
pos9 =  a(x,y);
       ...
       % Interaction between the virus and normal T-cells
...
a(x+1,y+1) = 0;
      ...
      % The supply of normal T-cells by the lymphatic system  (0->T) 
...
a(sub2ind(size(a),Subst1(:,1),Subst1(:,2)))=1;   %substitute the empty grid cells with normal T-cells (0->1)  
       ...
       %The growthrate of normal T-cells (0->T)
...
a(sub2ind(size(a),Subst7(:,1),Subst7(:,2)))=1;    %substitute the empty grid cells with normal T-cells (0->1)  
        ...
        %Converting latent T-cells to active T-cells (TL->TA)
...
a(sub2ind(size(a),Subst3(:,1),Subst3(:,2))) = 3;  %substitute the latent T-cells with active T-cells (2->3)  
          ...
          % Converting active T-cells to empty grid cells this is due to lysis (TA-> 0)
...
a(sub2ind(size(a),Subst4(:,1),Subst4(:,2))) = 0;  %substitute the active T-cells with empty grid cells (3->0)
         ...
         % Virus production due to lysis of TA (0->4) 
...
title ('Transection of blood vessel')
...
%% ploting the number of agents on the grid 
     
v1= sum(a==1,'all');     % suming up the number of normal T-cells after each iteration
V1=[V1 v1];              % saving the number of normal T-cells after each iteration in V1
v2= sum(a==2,'all');     % suming up the number of latent T-cells after each iteration
V2=[V2 v2];              % saving the number of latent T-cells after each iteration in V2
v3= sum(a==3,'all');     % suming up the number of active T-cells after each iteration
V3=[V3 v3];              % saving the number of active T-cells after each iteration in V3
v4= sum(a==4,'all');     % suming up the number of free virus after each iteration
